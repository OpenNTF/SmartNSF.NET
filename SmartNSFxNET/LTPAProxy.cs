...
{
    public class LTPAProxy
    {
        private SmartNSFProxyClient _snsfpClient;
        
        public LTPAProxy(SmartNSFProxyClient client)
        {
            this._snsfpClient =client;
        }
        public async Task<IActionResult> MakeProxyCall(HttpRequest req, string userName, ILogger log)
        {
            ...
            LTPAAnswer ltpaToken = await this._snsfpClient.GetLTPAToken(userName,log);
            this._snsfpClient.AddLTPACookie(request,ltpaToken.LtpaToken);
            try {
        ...
        public async Task<IActionResult> MakeBinaryProxyCall(HttpRequest req, string userName, ILogger log)
        {
            string action = req.Query["action"];
            log.LogInformation("Action is: " + action);
